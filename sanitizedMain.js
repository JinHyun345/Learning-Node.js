...
var app = http.createServer(function (request, response) {
    ...
    if (pathname === "/") {
        if (queryData.id === undefined) {
            ...
            fs.readdir("./data", function (err, filelist) {
                var fiteredId = path.parse(queryData.id).base;
                fs.readFile(`data/${fiteredId}`, "utf8", function (err, data) {
                    var control = `
                    <a href="/add">add</a>
                    ...
                    var Body = `<h3>${sanitizedTitle}</h3><p>${sanitizedData}</p>`;
                    var html = template.html(
                        title,
                        template.list(filelist),
                        Body,
        ...
        request.on("end", function () {
            ...
            var post = Object.assign({}, postpost);
            var id = post.id;
            var fiteredId = path.parse(id).base;
            fs.unlink(`data/${fiteredId}`, function (err) {
